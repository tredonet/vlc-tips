...
function decorate(verb: HttpVerb) {
  return function (target: object, propertyKey: string | symbol, descriptor: TypedPropertyDescriptor<any>) {
    ...
    descriptor.value = async function (req: NextApiRequest, res: NextApiResponse) {
      try {
        const parameters = parameterDecorators.map(({ location, name, index }) =>
          getParameterValue(req, res, {
            location,
            name,
            index,
          })
        ...
        const responseCode: number =
          Reflect.getMetadata(HTTP_CODE_TOKEN, target.constructor, propertyKey) ?? (returnValue ? 200 : 204);
