...
export const TipsListHeader = ({ category, onClick }: ListHeaderProps) => {
  return (
    <div key={category} className="bg-slate-200 py-1 px-4 rounded-xl text-black flex gap-2 select-none cursor-pointer hover:bg-slate-300" onClick={onClick}>
...
export const Tip = ({ tip, selected, onClick }: TipProps) => {
  ...
  return (
    <>
      <div key={tip.name} className="px-2 flex gap-2 items-center cursor-pointer min-h-8" onClick={onClick}>
        <FontAwesomeIcon icon={selected ? faAngleDown : faAngleRight} className="min-w-4" />
...
const TipDescription = ({ tip }: { tip: ITip }) => {
  return (
    <>
      ...
      <a href={tip.mapsUrl} target="_blank" className=" px-4 py-1 hidden sm:block cursor-pointer font-bold">
        Navigation &rarr;
      </a>
      <a className="cursor-pointer block sm:hidden" href={tip.mapsUrl} target="_blank">
        <Map zoom={13} center={[tip.geometry.lat, tip.geometry.lng]} className="w-full h-60" zoomControl={false}>
          <Marker tip={tip} />
        </Map>
      </a>
    </>
